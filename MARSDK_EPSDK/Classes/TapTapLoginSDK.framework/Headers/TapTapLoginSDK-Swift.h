...
#pragma clang attribute push(                                                                                          \
    __attribute__((external_source_symbol(language = "Swift", defined_in = "TapTapLoginSDK", generated_declaration))), \
...
+ (AccessToken *_Nonnull)buildWithKid:(NSString *_Nonnull)kid
                            tokenType:(NSString *_Nonnull)tokenType
                               macKey:(NSString *_Nonnull)macKey
                         macAlgorithm:(NSString *_Nonnull)macAlgorithm
                                scope:(NSString *_Nullable)scope SWIFT_WARN_UNUSED_RESULT;
...
/// 如果你的应用支持 iOS 13 Scene，需要在 <code>scene(_:, openURLContexts
/// URLContexts:)</code> 函数中调用 如果你的应用支持 AppDelegate 唤起，需要在
/// <code>application(_ app: , open url: )</code> 函数中调用
...
/// OC 的登录方法
+ (void)LoginWithScopes:(NSArray<NSString *> *_Nonnull)scopes
                handler:(void (^_Nonnull)(BOOL, NSError *_Nullable, TapTapAccount *_Nullable))handler;
@end
