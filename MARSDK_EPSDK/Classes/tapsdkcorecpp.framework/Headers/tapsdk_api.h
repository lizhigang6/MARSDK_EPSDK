...
extern "C"
{
     ...
     * 成功返回 0，失败返回 -1
     */
    TDSSDK_EXPORT_API int TdkOnAppStarted(const char *cfg, CommonVariablesGetter commonVariablesGetter,
    ...
    /**
     * 需要发送埋点日志时调用。
     * SDK 内部会整合初始化接口传入的公参、extraArgsFunc 返回的公参，以及 log
     * 里的业务参数， 然后再发送到 OpenLog 服务端
     *
     *   logStore 如：tapsdk、tapsdk-apm
     *   logContent 业务埋点日志，仅需传递业务参数，JSON
     * 格式：{"action":"xxx","open_id":"yyy","tds_user_id":"zzz"}
     */
    ...
    /**
     * 设置额外的时长模块日志参数，JSON K/V
     * 格式。每次调用这个接口时，会用最新得到的参数，替换原有参数。
     *
