...
/*
    业务接口前置校验协议，对外类实现该协议后，除了白名单中的方法(通过
   getWhiteListMethodArray 设置）外，所有静态方法都会在执行前调用
   verifyCurrentState ，并根据返回值判断是真正执行该接口还是返回默认值
 */
...
/// 白名单列表，白名单中的方法会直接执行，不通过 verifyCurrentState
/// 校验，建议把内部方法加入进来 默认会添加 load 方法
