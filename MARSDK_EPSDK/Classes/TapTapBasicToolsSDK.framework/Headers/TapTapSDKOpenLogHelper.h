...
typedef NS_ENUM(NSInteger, OpenLogEventActionType) { INIT, START, SUCCESS, FAIL, CANCEL };
// 事件属性参数
...
+ (void)reportBusinessLog:(TapTapSDKProjectType)project
...
///   - action: 埋点事件
///   - properties: 埋点参数，上报时参数 key 为 args
+ (void)reportTechnicalLog:(TapTapSDKProjectType)project
                    action:(NSString *)action
                properties:(nullable NSDictionary *)properties;
...
+ (NSString *)reportBusinessLog:(TapTapSDKProjectType)project
                     actionType:(OpenLogEventActionType)actionType
                     properties:(nullable NSDictionary *)properties;

/// 业务埋点 （上报 tapsdk表）
/// - Parameters:
///   - projcet:模块名称
///   - eventKey: 事件 KEY, , session_id 根据 event_key 管理
///   一次接口调用后可能内部会有多个事件并行执行，例如成就，所以使用该字段来区分不同事件
///   - actionType: 埋点事件类型
///   - properties: 埋点参数，上报时参数 key 为 args
/// - Returns: 该事件上报时携带的 session 值，当 actionType 为 init 时为 nil
+ (NSString *)reportBusinessLog:(TapTapSDKProjectType)project
                       eventKey:(nullable NSString *)eventKey
                     actionType:(OpenLogEventActionType)actionType
                     properties:(nullable NSDictionary *)properties;
