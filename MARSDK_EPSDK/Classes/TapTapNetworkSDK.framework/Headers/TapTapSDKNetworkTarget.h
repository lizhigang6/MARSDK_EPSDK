...
typedef void (^TapTapNetworkCallBack)(NSDictionary *_Nullable json, NSURLResponse *_Nullable response,
                                      NSError *_Nullable error);

// 重试策略枚举
typedef NS_ENUM(NSInteger, TapTapRetryStrategy) {
    // 默认情况三种情况会重试 1、HTTP Status Code 大于等于
    // 500。2、请求超时。3、无网络。
    ...
    // 指数退避策略：第一次间隔 2s 后重试，第二次间隔 4s，第三次间隔
    // 8s，直到最大间隔 600s。无最大重试次数限制。
...
typedef NS_ENUM(NSInteger, TapTapHTTPBodyDataType) { TapTapHTTPBodyDataTypeJSON, TapTapHTTPBodyDataTypeForm };
