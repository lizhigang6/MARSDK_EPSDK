...
//  4.1.3IR01B0320 1. 优化token获取流程。 减少交互。
//  使用者自行缓存accessCode（具备有效期）。 4.3.0IR01B0615 1.
//  socket通讯优化（GCD方案） 4.4.0IR01B0715 1.
//  预取号流程修改 2.增加读取idfa的功能 3.降低打点频率。 4.5.0IR01B1020 1.
//  关闭日志 2.错误码细化 4.6.0IR01B0105 1. http1.1规范
//  header部分头域兼容大小写，兼容http链接中缺少80端口问题。
//                 2. socket请求时，host头域追加端口号
//                 3. 仅数据网络开通和wifi双开时，都启动预取号计时器。
//                 4. 按照阿里建议， 超时时间可以设置x.x秒样式
//  4.7.0IR01B0331 1. 减少体积
//                 2.
//                 解决苹果appstore审核时，可能会对getaddrinfo域名解析接口触发本地设备连接授权弹窗进行驳回处理。
//  5.0.0IR01B0420 全https 5g取号支持
//  5.1.0IR01B0701 ipv6网络支持
//                 开启缓存模式支持
//                 解决偶发的1002验签错误bug
//                 未确保兼容性，建议讲wostore.cn
//                 10010.com域名及其子域名加入到http通信白名单中。
//                 解决不符合url规范的特殊url地址的socket网络访问bug
//  5.1.0IR01B0712 删除缓存相关代码
//  5.1.1IR01B0825 预取号请求的队列修改为并发队列。预取号请求日志添加开关控制。
//  5.1.2IR01B0110
//  解决高并发预取号请求可能导致偶发的崩溃问题。（但不建议高并发调用预取号接口）
...
/**
 ...
 2. 手机本地时间正确（北京时间）
 3.
 如果手机数据网络开启，但是预取号失败，可以尝试手动飞行模式开关一下，尝试让数据网络恢复正常。
 4. 物联网卡无法取号
 5. 海外，港澳地区无法取号
